...
To run these migrations within the context of the application, simply run `bin/rake
...
```bash
bin/rake db:migrate SCOPE=blorgh
```
...
```bash
bin/rake db:migrate SCOPE=blorgh VERSION=0
```
...
The `bin/rake db:migrate` command needs to be run here to ensure that our
...
your engine assets when `bin/rake assets:precompile` is triggered.
